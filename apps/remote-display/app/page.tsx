"use client";
import { useState } from "react";
import Link from "next/link";
import { Button } from "@repo/ui/components/button";
import {
  Card,
  CardContent,
  CardHeader,
  CardTitle,
} from "@repo/ui/components/card";
import { Input } from "@repo/ui/components/input";
import { Monitor, Smartphone, Plus } from "lucide-react";

export default function Home() {
  const [sessionId, setSessionId] = useState("");

  const generateSessionId = () => {
    return Math.random().toString(36).substring(2, 8).toUpperCase();
  };

  const createNewSession = () => {
    const newId = generateSessionId();
    setSessionId(newId);
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-br from-background to-muted/30">
      <div className="text-center mb-12">
        <h1 className="text-4xl sm:text-6xl font-bold text-foreground mb-4 font-sans">
          📺 Remote Display
        </h1>
        <p className="text-xl text-muted-foreground font-mono max-w-2xl">
          Upload content and control what displays on the big screen from your
          phone
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl w-full">
        {/* Display Screen Card */}
        <Card className="p-6">
          <CardHeader className="text-center pb-4">
            <Monitor className="w-16 h-16 mx-auto mb-4 text-primary" />
            <CardTitle className="text-2xl">Display Screen</CardTitle>
            <p className="text-muted-foreground">
              Open this on your main display/TV screen
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Input
                placeholder="Enter session ID"
                value={sessionId}
                onChange={(e) => setSessionId(e.target.value)}
              />
              <Button variant="outline" onClick={createNewSession}>
                <Plus className="w-4 h-4" />
              </Button>
            </div>
            <Link
              href={`/display${sessionId ? `?session=${sessionId}` : ""}`}
              className="block"
            >
              <Button className="w-full h-12 text-lg">Open Display</Button>
            </Link>
            <p className="text-sm text-muted-foreground text-center">
              Full-screen content display with no scrollbars
            </p>
          </CardContent>
        </Card>

        {/* Remote Control Card */}
        <Card className="p-6">
          <CardHeader className="text-center pb-4">
            <Smartphone className="w-16 h-16 mx-auto mb-4 text-primary" />
            <CardTitle className="text-2xl">Remote Control</CardTitle>
            <p className="text-muted-foreground">
              Open this on your phone or tablet
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Input
                placeholder="Enter session ID"
                value={sessionId}
                onChange={(e) => setSessionId(e.target.value)}
              />
              <Button variant="outline" onClick={createNewSession}>
                <Plus className="w-4 h-4" />
              </Button>
            </div>
            <Link
              href={`/remote${sessionId ? `?session=${sessionId}` : ""}`}
              className="block"
            >
              <Button variant="outline" className="w-full h-12 text-lg">
                Open Remote
              </Button>
            </Link>
            <p className="text-sm text-muted-foreground text-center">
              Upload content and control what displays
            </p>
          </CardContent>
        </Card>
      </div>

      <div className="mt-12 text-center max-w-2xl">
        <h2 className="text-2xl font-semibold mb-4">How it works</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mb-2">
              <span className="text-primary font-bold">1</span>
            </div>
            <p className="text-muted-foreground">Create or join a session</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mb-2">
              <span className="text-primary font-bold">2</span>
            </div>
            <p className="text-muted-foreground">Upload content from remote</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mb-2">
              <span className="text-primary font-bold">3</span>
            </div>
            <p className="text-muted-foreground">Select to display instantly</p>
          </div>
        </div>
      </div>
    </div>
  );
}
